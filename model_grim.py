...
class VisionTransformer(tf.keras.Model):
    def __init__(self,image_size,patch_size,num_layers,num_classes,d_model,num_heads,mlp_dim,channels,dropout):
        ...
        self.patch_dim = channels * patch_size ** 2#每个patch打平后的向量维度 = 16
    ...
    def extract_patches(self, images):#提取patch
        ...
        #extract_patches将patch放在最后一个维度（通道）上
        patches = tf.image.extract_patches(#将图片分割为多个子图片,并且返回一个4D张量，与images一样
            images=images,
            sizes=[1, self.patch_size, self.patch_size, 1],#分割成的patches图片大小
...
def grim_top_right(matrix):
    '''
    求一个矩阵的右上角元素矩阵
    :param matrix: 传入的格拉姆矩阵——对称的
    :return: 返回右上角元素组成的numpy数组
    '''
    features=[]
    gram_len=matrix.shape[1]
    for row in range(gram_len):
        for clo in range(gram_len):
            clos=clo+row
            if(clos>gram_len-1):
                break
            features.append((matrix[row][row+clo]))
